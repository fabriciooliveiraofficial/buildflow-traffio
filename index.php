...
// 1. Check for build-hash.txt (generated by deployment pipeline)
$buildHashFile = __DIR__ . '/build-hash.txt';
if (file_exists($buildHashFile)) {
    $gitHash = substr(trim(file_get_contents($buildHashFile)), 0, 10);
}

// 2. Fallback to local git directory (for dev/local environment)
...
// 3. Fallback if git/build-hash is not available
if ($gitHash === 'no-git-hash') {
